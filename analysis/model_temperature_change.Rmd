...
```{r message=FALSE}
climdiv_national <- read_csv("input/processed/climdiv_national_2020.csv")

model_national <- 
  climdiv_national %>% 
  group_by(NA) %>% # our fancy dplyr function expects to receive groups
  modelAnnualTempChg() 
```
...
```{r message=FALSE}
climdiv_state <- read_csv("input/processed/climdiv_state.csv")
climdiv_state_year <- read_csv("input/processed/climdiv_state_2020.csv")
...
head(model_state, 48) %>% 
  ...
  dplyr::select(State, `Temperature change`) %>% 
  knitr::kable()
```
...
model_state_seasonal <-
  ...
  left_join(dplyr::select(model_state, fips, tempchg_c), by="fips") %>% 
  left_join(statefips, by=c("fips"="STATE")) %>%
  rename(Annual=tempchg_c) %>% 
  arrange(-Annual) %>% 
  dplyr::select(-STUSAB, -STATENS) %>% 
  dplyr::select(fips, STATE_NAME, Annual, everything())

write_csv(model_state_seasonal, "input/processed/model_state.csv")
  
head(model_state_seasonal, 10) %>% 
  mutate(State=STATE_NAME, `Winter temperature change`=paste0(fmt_c(Winter), " (", fmt_f(Winter * 1.8), ")")) %>% 
  dplyr::select(State, `Winter temperature change`) %>% 
  knitr::kable()
```
...
```{r}
climdiv_county <- read_csv("input/processed/climdiv_county.csv")
climdiv_county_year <- read_csv("input/processed/climdiv_county_2020.csv")
...
model_county_seasonal <-
  ...
  left_join(dplyr::select(model_county, fips, tempchg_c), by="fips") %>% 
  left_join(countynames, by="fips") %>% 
  rename(Annual=tempchg_c) %>% 
  arrange(-Annual) %>% 
  dplyr::select(fips, CTYNAME, STNAME, Annual, everything())

write_csv(model_county_seasonal, "input/processed/model_county.csv")
```
...
countyshp <- 
  st_read("input/shapefiles/cb_2018_us_county_20m/cb_2018_us_county_20m.shp")

# ALAND is square meters
countyshp_data <-
  countyshp %>%
  mutate(fips=paste0(STATEFP, COUNTYFP)) %>%
  left_join(dplyr::select(model_county, fips, tempchg, tempchg_c, POPESTIMATE2018), by="fips") %>%
...
countyshp %>% 
  mutate(fips=paste0(STATEFP, COUNTYFP)) %>%
  anti_join(dplyr::select(model_county, fips, tempchg), by="fips")# %>% View()

# No data without a county shape
dplyr::select(model_county, fips, tempchg) %>% 
  anti_join(mutate(countyshp, fips=paste0(STATEFP, COUNTYFP)), by="fips")# %>% View()

st_write(countyshp_data, "input/processed/model_county.geojson", delete_dsn = TRUE)
```